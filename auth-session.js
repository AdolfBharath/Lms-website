(function () {
  ...
  function storeSession(profile, authUser) {
  ...
  async function profileFromCurrentAuth() {
    ...
    return storeSession(profile, data.user);
  }
  ...
  async function signInWithPassword(email, password) {
    ...
    if (legacyProfile) {
      return storeSession(legacyProfile, { id: legacyProfile.auth_user_id || legacyProfile.id, email: legacyProfile.email });
    }
  ...
  async function legacyPasswordLogin(email, password) {
    ...
    if (error) {
      if (/function|schema cache|not found|permission|denied|42501|PGRST202/i.test(error.message || error.code || "")) {
        throw new Error("Legacy login is not enabled in Supabase. Run the legacy login setup or migrate this account to Supabase Auth.");
      }
      return null;
    }
